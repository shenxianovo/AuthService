  ...
  <div>
    <Tabs default-value="signup" class="mb-6">
      <TabsList class="w-full">
        <TabsTrigger value="signin" class="flex-1" @click="$router.push('/login')">{{ t('common.signIn') }}</TabsTrigger>
        <TabsTrigger value="signup" class="flex-1">{{ t('common.signUp') }}</TabsTrigger>
      </TabsList>
    </Tabs>
...
async function handleRegister() {
  ...
  try {
    ...
    error.value = translateApiError(e, t('register.failed'))
  } finally {
    loading.value = false
  }
}
