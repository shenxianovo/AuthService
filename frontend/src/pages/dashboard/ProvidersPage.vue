        ...
        <div v-if="providers.length" class="flex flex-wrap gap-3">
          ...
          >
            ...
            <button
              ...
              :disabled="loading"
              :title="t('providers.unlink')"
              @click="handleUnlink(p.provider!)"
            >&#x2715;</button>
          </div>
        </div>
        <p v-else class="text-sm text-muted-foreground">{{ t('providers.none') }}</p>
...
async function handleUnlink(provider: string) {
  ...
  try {
    ...
    success.value = t('providers.unlinked', { provider })
    await userStore.fetch()
  } catch (e: unknown) {
    error.value = translateApiError(e, t('providers.unlinkFailed'))
  } finally {
    loading.value = false
  }
}
...
function consumeBindResult() {
  ...
  if (bound) success.value = t('providers.linked', { provider: bound })
  if (bindError) error.value = t('providers.linkFailed', { error: bindError })
