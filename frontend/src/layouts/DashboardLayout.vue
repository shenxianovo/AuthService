    ...
    <div class="flex-1 md:ml-60 flex flex-col min-h-screen">
      <header class="sticky top-0 z-[100] h-[60px] px-4 md:px-8 flex items-center justify-end bg-glass/80 backdrop-blur-glass border-b border-border">
        ...
        <div class="flex items-center gap-2.5">
          ...
          <span class="text-sm font-medium text-foreground">{{ userStore.userInfo.value?.displayName ?? t('nav.user') }}</span>
        </div>
      </header>
...
const baseNavItems = [
  { to: '/dashboard/profile', labelKey: 'nav.profile', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>' },
  { to: '/dashboard/emails', labelKey: 'nav.email', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>' },
  { to: '/dashboard/providers', labelKey: 'nav.linkedAccounts', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>' },
  { to: '/dashboard/api-keys', labelKey: 'nav.apiKeys', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>' },
  { to: '/dashboard/security', labelKey: 'nav.security', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>' },
]

// Visibility hint only — every admin request is re-authorized server-side.
const adminNavItem = {
  to: '/dashboard/oidc-clients',
  labelKey: 'nav.oidcClients',
  icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
}

const navItems = computed(() =>
  userStore.userInfo.value?.role === 'Admin' ? [...baseNavItems, adminNavItem] : baseNavItems)
